...
def execute_remote_container(service: str, host: str, cmd: List[str]):
    ...
    res = subprocess.run(["ssh", host, "docker", "exec", f"hotel_{service.lower()}"] + cmd, capture_output=True)
...
def copy_remote_container(service: str, host: str, local_path: str, remote_path: str):
    ...
    res = subprocess.run(["rsync", "-avz", local_path, f"{host}:/tmp"], capture_output=True)
    error_handling(res, f"Error when rsync-ing file {local_path}")
    execute_remote_host(host, ["docker", "cp", f"/tmp/{filename}", f"hotel_{service.lower()}:{remote_path}"])
