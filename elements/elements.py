...
class ACL(Element):
    ...
    def init_state(self):
        ...
        self.cursor.execute('''CREATE TABLE acl (
                        name VARCHAR(255),
        ...
        self.cursor.executemany("INSERT INTO acl (permission, name) VALUES (?, ?)", data)

        if self.verbose: self.print_table('acl')
    ...
    def egress_process(self, input_table_name):
        ...
        self.cursor.execute('''CREATE TABLE output AS
        ...
        if self.verbose: self.print_table('output')
...
class Logging(Element):
    ...
    def init_state(self):
        ...
        ''')    
        if self.verbose: self.print_table("rpc_events")
    ...
    def egress_process(self, input_table_name):
        ...
        self.cursor.execute('''INSERT INTO rpc_events (timestamp, src, dst, value) 
            SELECT CURRENT_TIMESTAMP, src, dst, name || " " || message FROM {};'''.format(input_table_name))

        self.cursor.execute('''DROP TABLE IF EXISTS output''')
        self.cursor.execute('''CREATE TABLE output AS SELECT * from {}'''.format(input_table_name))
...
class RateLimit(Element):
    ...
    def init_state(self):
        ...
        if self.verbose: self.print_table("token_bucket")
