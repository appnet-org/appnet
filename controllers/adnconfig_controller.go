...
func mrpc_init_setup(ctx context.Context, controlPlaneID string) {
	...
	RunCommand("sudo", "docker", "exec", controlPlaneID, "cargo", "run", "--release", "--bin", "addonctl", "--", "--config", "eval/policy/chain/phase1/receiver_attach.toml", "--pid", strconv.Itoa(serverPid), "--sid", "1")
	RunCommand("sudo", "docker", "exec", controlPlaneID, "cargo", "run", "--release", "--bin", "addonctl", "--", "--config", "eval/policy/chain/phase1/ratelimit_attach.toml", "--pid", strconv.Itoa(clientPid), "--sid", "1")		
	RunCommand("sudo", "docker", "exec", controlPlaneID, "cargo", "run", "--release", "--bin", "addonctl", "--", "--config", "eval/policy/chain/phase1/logging_attach.toml", "--pid", strconv.Itoa(clientPid), "--sid", "1")
}	
...
func mrpc_after_migration_unsafe(ctx context.Context, controlPlaneID string) {
	...
	RunCommand("sudo", "docker", "exec", controlPlaneID, "cargo", "run", "--release", "--bin", "addonctl", "--", "--config", "eval/policy/chain/phase2/receiver_detach.toml", "--pid", strconv.Itoa(serverPid), "--sid", "1")		
	time.Sleep(time.Second)
	RunCommand("sudo", "docker", "exec", controlPlaneID, "cargo", "run", "--release", "--bin", "addonctl", "--", "--config", "eval/policy/chain/phase2/sender_attach.toml", "--pid", strconv.Itoa(clientPid), "--sid", "1")
}
